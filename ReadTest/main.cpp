...
int main(int, char **)
{
    ...
    struct dirent *ent;
    DIR *pDir = opendir("/home/okui/stepcode/usr/ReadTest/usrdata/");
    if (pDir == NULL)
    {
        std::cerr << "Step File Folder Not Found" << std::endl;
        return 1;
    }

    while ((ent = readdir(pDir)) != NULL) {
        string path = ent->d_name;
        string regex = "\\w+\\.[stp|step]";

        std::smatch smatch;
        if( std::regex_search(path, smatch, std::regex(regex))) {
            ...
            STEPfile *sfile = new STEPfile(*registry, *instance_list, "/home/okui/stepcode/usr/ReadTest/usrdata/" + path, false);
