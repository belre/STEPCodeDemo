	...
	{
		{
			...
			STEPfile* sfile = new STEPfile(*registry, *instance_list, "F:\\StepData\\BSP35B20-N-12.stp", false);
					...
					for (int pos = 0; pos < entrycount; pos++) {
						...
						if (nonreftype == STRING_TYPE) {
							...
							if (instobj != NULL) {
								...
								if (instobj->CurrentUnderlyingType() == config_control_design::e_axis2_placement_2d) {
								...
								if (instobj->CurrentUnderlyingType() == config_control_design::e_axis2_placement_3d) {
